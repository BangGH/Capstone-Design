...
# Firebase 초기화
cred = credentials.Certificate('c:/20200792/testkey.json')
...
while True:
    ...
    if frame_count % frame_skip == 0:
        for obj in qr_results:
            ...
            if data.startswith("A=3,B=2,C=1"):  # 조건 예시: "A=3,B=0,C=2"로 시작하면
                update_count('A', -3)
                update_count('B', -2)
                update_count('C', -1)
                time.sleep(1)
            if data.startswith("A=3,B=0,C=2"):  # 조건 예시: "A=3,B=0,C=2"로 시작하면
                update_count('A', -3)
                update_count('B', 0)
                update_count('C', -2)
                time.sleep(1)
            if data.startswith("A=0,B=1,C=3"):  # 조건 예시: "A=3,B=0,C=2"로 시작하면
                update_count('A', 0)
                update_count('B', -1)
                update_count('C', -3)
                time.sleep(1)
        ...
        for obj in dm_results:
            ...
            if data.startswith("88"):  # 조건 예시: "88"로 시작하면
